defmodule AnotherTrackingToolWeb.UserAuthTest do
  use AnotherTrackingToolWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias AnotherTrackingTool.Accounts
  alias AnotherTrackingTool.Accounts.Scope
  alias AnotherTrackingToolWeb.UserAuth

  import AnotherTrackingTool.AccountsFixtures

  @remember_me_cookie "_another_tracking_tool_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AnotherTrackingToolWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "writes a cookie if remember_me was set in previous session", %{conn: conn, user: user} do
      ...
      conn =
        ...
        |> Map.replace!(
          :secret_key_base,
          AnotherTrackingToolWeb.Endpoint.config(:secret_key_base)
        )
        |> fetch_cookies()
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      AnotherTrackingToolWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: AnotherTrackingToolWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        AnotherTrackingToolWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
